...
## G28 · Поле токсичности вида `toxic` 🟡
- **Экран:** 12 Каталог (бейдж 🐈), 20 Карточка вида (баннер токсичности).
- **Нужно:** признак токсичности вида для бейджа/баннера (#128).
- **Сейчас:** подтверждено сверкой 2026-05-28 — `SpeciesDto` (`/species`, `/species/{id}`)
  поля `toxic` НЕ содержит (есть `careDifficulty/lightPreference/*Days/description/facts`).
- **Предложение:** `toxic: bool` (или `toxicity: enum`) в `SpeciesDto`.
- **Заглушка мобилки:** каталог 12 уже живёт без бейджа; карточка вида 20 (готова к работе)
  баннер токсичности рисует заглушкой/скрывает, пока поля нет.
