...
3. Открой `design/PlantCare.html` в браузере — это интерактивный прототип всех 38 экранов в светлой и тёмной теме.
4. `screenshots/` — эталонные рендеры каждого экрана (light + dark). Сверяй реализацию с ними; карта файлов — в `screenshots/INDEX.md`.
5. **`VERIFY.md`** — плейбук сверки: как запустить, как снять сопоставимый кадр и чек‑лист + матрица прогресса по всем экранам. Используй после реализации каждого экрана.
6. **`MOTION.md`** — спецификация микровзаимодействий (длительности, кривые, haptics). Визуальный референс — `design/Motion.html` (зацикленные демо).
7. **`A11Y.md`** — доступность (Dynamic Type, хит-таргеты, VoiceOver/TalkBack, контраст) и устойчивость к данным. Визуальный референс — `design/Robustness.html`.
...
### 11 · График (`screens-week-variants.jsx → WeekCalA`, экспортируется как `WeekCalendarScreen`)
**Редизайн.** Бывший матричный календарь заменён на agenda‑паттерн: горизонтальный день‑селектор
с точками нагрузки → список задач выбранного дня, сгруппированный по времени суток (Утро/Вечер/Сделано).
Полная спецификация — в **`screen-11-redesign.md`**. Варианты B/C остались в файле как запасные.
...
├── README.md                ← этот файл
├── screen-11-redesign.md     ← полная спецификация редизайна экрана 11 (agenda)
├── VERIFY.md                ← плейбук сверки реализации со скриншотами (+ a11y и стресс-чеклисты)
├── MOTION.md                ← спецификация микровзаимодействий (длительности/кривые/haptics)
├── A11Y.md                  ← доступность + устойчивость к данным (Dynamic Type, screen reader, контраст)
...
│   └── dark/    (43 PNG)
└── design/
    ├── PlantCare.html       ← открыть в браузере, увидеть все 38 экранов
    ...
    ├── screens-week-variants.jsx ← 11 График — РЕДИЗАЙН (WeekCalA = продакшен; WeekCalB/C — запасные)
