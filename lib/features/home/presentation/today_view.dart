...
/// (`task.isDone`) не попадают в секции и никогда не помечаются [overdue] —
/// после отметки задача исчезает с экрана (UX «вариант 1»).
@freezed
...
/// backend gap G11 закрыт) НЕ попадают в [groups] — после отметки задача сразу
/// исчезает с экрана (UX «вариант 1»). [doneCount] / [totalCount] всё равно
/// считаются по полному списку и питают прогресс-карточку «X из N выполнено».
/// (Список [doneItems] остаётся в модели как источник `doneCount`/будущих
/// нужд, но на экране 03 больше не рендерится.)
///
...
abstract class TodayView with _$TodayView {
  const factory TodayView({
    ...
    /// Выполненные сегодня задачи, отсортированы по `doneAt` убыванию
    /// (последняя — сверху). На экране 03 больше НЕ рендерятся (отмеченная
    /// задача исчезает); поле сохранено как источник `doneCount` и на будущее.
  ...
  /// Есть ли выполненные задачи (числитель прогресса; секция «Выполнено»
