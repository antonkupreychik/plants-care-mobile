...
abstract class ShoppingClient {
  ...
  /// Очистить отмеченные позиции (bulk).
  ///
  ...
  ///
  /// [checked] - Фильтр. Единственное допустимое значение — `true` (удалить только отмеченные).
  @DELETE('/api/v1/shopping/clear')
  Future<void> clearCheckedShoppingItems({
    @Query('checked') required bool checked,
    @Extras() Map<String, dynamic>? extras,
  });
