...
/// Усекает помеченные query-параметры и поля тела до date-only (`YYYY-MM-DD`).
///
...
/// → `2026-05-20T00:00:00.000`. Backend на таком значении отдаёт HTTP 500/400
/// (ждёт `2026-05-20`). См. docs/BACKEND-GAPS.md.
///
/// Opt-in по образцу `authScopeExtra`: data source помечает нужные ключи через
/// [dateOnlyQueryExtra] (query) и/или [dateOnlyBodyExtra] (поля JSON-тела),
/// интерсептор читает [kDateOnlyQueryKeysExtraKey] / [kDateOnlyBodyKeysExtraKey]
/// из `options.extra` и усекает только перечисленные значения. Снимается, когда
/// кодген/спека починят сериализацию даты — тогда хелперы и интерсептор удаляются.
class DateQueryInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    final queryKeys = options.extra[kDateOnlyQueryKeysExtraKey];
    if (queryKeys is Iterable<String>) {
    ...
    final bodyKeys = options.extra[kDateOnlyBodyKeysExtraKey];
    final body = options.data;
    if (bodyKeys is Iterable<String> && body is Map) {
      for (final key in bodyKeys) {
        if (!body.containsKey(key)) continue;
        final dateOnly = _toDateOnly(body[key]);
        if (dateOnly != null) {
          body[key] = dateOnly;
        }
      }
